...
def final_fix_confirmed_root():
    host = "ssh-quickcombo.alwaysdata.net"
    user = "quickcombo"
    password = "Dinesh@061004"
    
    # THE DEFINITIVELY PROVEN LIVE DIRECTORY (CONTAINING debug_db)
...
urlpatterns = [
    # ADMIN DASHBOARD (CONFIRMED ROOT)
...
'''
    ...
    try:
        ...
        print(f"Connected! Repairing TRUE ROOT: {LIVE_ROOT}...")
        ...
        print("\\n--- Purging Cache & Hard Restart ---")
        ssh.exec_command(f"find {LIVE_ROOT} -name '__pycache__' -type d -exec rm -rf {{}} +")
        ssh.exec_command(f"find {LIVE_ROOT} -name '*.pyc' -delete")
